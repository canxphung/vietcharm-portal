...
export class NearbyPlacesComponent {
  ...
  readonly active = computed(() => this.places().find((place) => place.id === this.activeId()) ?? null);
  ...
  async addReview(place: NearbyPlace): Promise<void> {
    if (!this.reviewerName().trim() || !this.reviewComment().trim() || this.submittingReview()) return;
