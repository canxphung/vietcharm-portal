...
router.post('/', async (req, res) => {
  try {
    ...
    const created = await VehicleModel.create({ _id: id, ...rest });
...
router.patch('/:id', async (req, res) => {
  try {
    ...
    const doc = await VehicleModel.findByIdAndUpdate(req.params.id, updates, {
      new: true,
      runValidators: true,
    });
    if (!doc) return res.status(404).json({ message: 'Vehicle not found' });
...
router.delete('/:id', async (req, res) => {
  try {
    const doc = await VehicleModel.findByIdAndDelete(req.params.id);
    if (!doc) return res.status(404).json({ message: 'Vehicle not found' });
    res.status(204).send();
  } catch (error) {
    res.status(500).json({ message: error instanceof Error ? error.message : 'Server Error' });
